...
COMFY_DIR = Path("/Users/Shared/p/ingenierias-lentas/narraciones-de-economicos/tools/ComfyUI")
...
OUT_ROOT = CASE_DIR  # we'll post filename_prefix rooted in outputs; then move if needed
...
def wait_for_prompt(prompt_id: str, poll_s: float = 1.0, timeout_s: int = 900):
    t0 = time.time()
...
def main(limit: int | None = None):
    ...
    for i, line in enumerate(lines, start=1):
        ...
        # Map from prompts.jsonl -> workflow nodes
        ...
        # Checkpoint name expected to exist in ComfyUI/models/checkpoints
        wf["4"]["inputs"]["ckpt_name"] = spec.get("checkpoint", wf["4"]["inputs"]["ckpt_name"])

        # Output naming: ComfyUI saves under ComfyUI/output/
        # We keep filename_prefix aligned with requested out_path (sans extension)
        ...
        payload = {"prompt": wf}
        try:
            res = comfy_post_json("/prompt", payload)
...
if __name__ == "__main__":
    # set LIMIT env var to do a small batch: LIMIT=5 python generate_images_comfy.py
